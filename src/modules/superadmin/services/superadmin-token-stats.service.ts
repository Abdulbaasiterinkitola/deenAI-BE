...
export class SuperadminTokenStatsService {
  ...
  async getUsageTrend(opts: { period: 'day' | 'week' | 'month'; startDate?: string; endDate?: string }): Promise<TimeSeriesDto> {
    const { period, startDate, endDate } = opts;
    const start = this.parseDateOrNull(startDate) || new Date(Date.now() - 1000 * 60 * 60 * 24 * 30);
    ...
    try {
      const trunc =
        period === 'month' ? `date_trunc('month', created_at)` :
        period === 'week' ? `date_trunc('week', created_at)` :
        `date_trunc('day', created_at)`;

      const rows: Array<{ bucket: string; input: string; output: string; total: string }> = await this.dataSource.query(
        `
      ...
      const granularity: 'hour' | 'day' | 'week' = period === 'month' ? 'week' : period;
  ...
  async getTopUsers(opts: { limit: number; period: 'all' | '7d' | '30d' }): Promise<TopUserDto[]> {
  ...
  async getBreakdown(): Promise<BreakdownDto> {
    try {
      const inputRow = await this.dataSource.query(`SELECT COALESCE(SUM(input_tokens),0) AS sum FROM token_usage`);
      const outputRow = await this.dataSource.query(`SELECT COALESCE(SUM(output_tokens),0) AS sum FROM token_usage`);
      const totalRequestsRow = await this.dataSource.query(`SELECT COUNT(*) AS c FROM token_usage`);
      ...
      const avgPerRequest = totalRequests ? (input + output) / totalRequests : 0;
  ...
  async getByPlan(): Promise<PlanStatsDto[]> {
    try {
      ...
      const totalAllRow = await this.dataSource.query(`SELECT COALESCE(SUM(input_tokens + output_tokens),0) AS total FROM token_usage`);
      ...
      return rows.map((r: any) => ({
        ...
        percentageOfTotal: totalAll ? (Number(r.total_tokens || 0) / totalAll) * 100 : 0,
      }));
    } catch (err) {
  ...
  async getUserDetail(opts: { userId: string; page: number; limit: number }): Promise<UserDetailDto> {
    ...
    try {
      ...
      const billingStart = billingStartRow?.[0]?.billing_start ? new Date(billingStartRow[0].billing_start) : (() => { const d = new Date(); d.setUTCDate(d.getUTCDate() - 30); return d; })();
      ...
      const points: TimeSeriesPoint[] = historyRows.map((r: any) => ({
  ...
  async getTimeSeries(opts: { granularity: 'hour' | 'day' | 'week'; startDate?: string; endDate?: string }): Promise<TimeSeriesDto> {
    const { granularity, startDate, endDate } = opts;
    const start = this.parseDateOrNull(startDate) || new Date(Date.now() - 1000 * 60 * 60 * 24 * 7);
    const end = this.parseDateOrNull(endDate) || new Date();

    if (start > end) throw new CustomHttpException('Invalid date range', 400);

    try {
      const trunc = granularity === 'hour' ? `date_trunc('hour', created_at)` : granularity === 'week' ? `date_trunc('week', created_at)` : `date_trunc('day', created_at)`;
      const rows: Array<{ bucket: string; input: string; output: string; total: string }> = await this.dataSource.query(
        `
        SELECT ${trunc} as bucket,
          COALESCE(SUM(input_tokens),0)::bigint AS input,
