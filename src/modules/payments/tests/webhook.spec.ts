...
describe('PaymentsWebhookService', () => {
    ...
    beforeEach(async () => {
        const module: TestingModule = await Test.createTestingModule({
            providers: [
                ...
                { provide: getRepositoryToken(WebhookLog), useValue: mockLogRepository },
                { provide: getRepositoryToken(PaymentTransaction), useValue: mockTransactionRepository },
            ],
        }).compile();
    ...
    describe('verifyGoogleSignature', () => {
        it('should throw UnauthorizedException if header is missing', async () => {
            await expect(service.verifyGoogleSignature('')).rejects.toThrow(UnauthorizedException);
        });

        it('should verify signature with local public key', async () => {
            const token = jwt.sign({ foo: 'bar' }, testPrivateKey, { algorithm: 'RS256' });
            const authHeader = `Bearer ${token}`;

            await expect(service.verifyGoogleSignature(authHeader)).resolves.not.toThrow();
        });
        ...
        it('should throw if signature is invalid', async () => {
            ...
            await expect(service.verifyGoogleSignature(authHeader)).rejects.toThrow(UnauthorizedException);
        });
    });
    ...
    describe('processGoogleWebhook', () => {
        it('should process RENEWED notification and record transaction', async () => {
            ...
            mockSubscriptionsService.getUserByGoogleToken.mockResolvedValue({ id: 'user123' });
            mockSubscriptionsService.getPlanByProductId.mockResolvedValue({ id: 'plan123' });
            ...
            expect(mockSubscriptionsService.getUserByGoogleToken).toHaveBeenCalledWith('token123');
            expect(mockSubscriptionsService.getPlanByProductId).toHaveBeenCalledWith('google', 'prod123');
            expect(mockSubscriptionsService.changePlan).toHaveBeenCalledWith('user123', 'plan123');
    ...
    describe('processAppleWebhook', () => {
        it('should process DID_RENEW notification and record transaction', async () => {
            ...
            expect(mockSubscriptionsService.getPlanByProductId).toHaveBeenCalledWith('apple', 'prod123');
            expect(mockSubscriptionsService.changePlan).toHaveBeenCalledWith('user123', 'plan123');
            expect(mockLogRepository.save).toHaveBeenCalledWith(expect.objectContaining({
                provider: 'apple',
